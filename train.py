...
def main(params):
    ...
    best_model, loss_history, train_acc, val_acc = trainer.train(poems_train, labels_train, poems_val, labels_val,
                                                                 model, loss_function,
                                                                 learning_rate=params['learning_rate'],
                                                                 learning_rate_decay=params['decay_rate'],
                                                                 update='rmsprop', sample_batches=True,
                                                                 batch_size=params['batch_size'],
                                                                 num_epochs=params['max_epochs'],
                                                                 decay_after=params['decay_after'],
                                                                 verbose=True, drop_prob1=params['drop_prob_layer1'],
                                                                 drop_prob2=params['drop_prob_layer2'],
                                                                 checkpoint_output_dir=params['checkpoint_output_dir'],
                                                                 max_val=params['eval_max'],
                                                                 fappend=params['fappend'],
                                                                 iter_to_update=params['iter_to_update'])
...
if __name__ == "__main__":
    ...
    parser.add_argument('-d', '--dataset', dest='dataset', default='haikus.csv',
                        help='dataset: cvs filename of the poems inside the data directory')
    parser.add_argument('-o', '--checkpoint_output_directory', dest='checkpoint_output_dir', type=str, default='cv/',
    ...
    parser.add_argument('--decay_after', dest='decay_after', type=int, default=1,
                        help='number of epochs to begin decaying learning rate after')
    parser.add_argument('-b', '--batch_size', dest='batch_size', type=int, default=100, help='batch size')
    parser.add_argument('--eval_max', dest='eval_max', type=int, default=-1,
                        help='batch size for validation testing. For efficiency we can use a smaller number of samples to get validation error')
    parser.add_argument('--drop_prob_layer1', dest='drop_prob_layer1', type=float, default=0.5,
                        help='what dropout to apply in first layer of RNN')
    parser.add_argument('--drop_prob_layer2', dest='drop_prob_layer2', type=float, default=0.5,
                        help='what dropout to apply in second layer of RNN')
    parser.add_argument('-n', '--num_layers', dest='num_layers', type=int, default=2,
                        help='number of hidden layers. can be either 1 or 2')
    parser.add_argument('-t', '--train_frac', dest='train_frac', type=float, default=.95,
                        help='fraction of data in the training set, should be between 0 and 1')
    parser.add_argument('--fappend', dest='fappend', type=str, default='baseline',
                        help='append this string to checkpoint filenames')
    parser.add_argument('--iter_to_update', dest='iter_to_update', type=int, default=1,
                        help='how often to display progress')
    parser.add_argument('-i', '--init_model', dest='init_model', default=None,
                        help='location of initial model to start training, default is random model')
