...
class RNNClassifierTrainer(object):
    ...
    def train(self, X, y, X_val_list, y_val_list,
        ...
        for it in xrange(num_iters):
            ...
            cost, grads = loss_function(X_batch, y_batch, batch_mask, model, batch_num_chars, reg,
                                        drop_prob1=drop_prob1, drop_prob2=drop_prob2)
            ...
            if first_it or epoch_end or acc_check:
                ...
                X_train_mat, train_mask, train_num_chars, y_train_mat = prep.poem_batch_to_tensor(X_train_subset_list,
                                                                                                  y_train_subset_list)
                ...
                # take a sample of the validation data if we are batching
                    ...
                    X_val_mat, val_mask, val_num_chars, y_val_mat = prep.poem_batch_to_tensor(X_val_subset_list,
                                                                                              y_val_subset_list)
                    scores_val = loss_function(X_val_mat, model=model, mask=val_mask, num_chars=val_num_chars)
                    val_loss, _ = loss_function(X_val_mat, y_val_mat, model=model, mask=val_mask,
                                                num_chars=val_num_chars)
                    y_pred_val = np.argmax(scores_val, axis=-1)
